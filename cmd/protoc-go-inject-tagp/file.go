...
func parseFile(inputPath string, src interface{}, xxxSkip []string, omit bool) (areas []textArea, err error) {
	...
	for _, decl := range f.Decls {
		...
		for _, field := range structDecl.Fields.List {
			...
			if useDefault && field.Tag != nil {
				currentTag := field.Tag.Value
				area := textArea{
					Start:      int(field.Pos()),
					End:        int(field.End()),
					CurrentTag: currentTag[1 : len(currentTag)-1],
					InjectTag:  defaultTag(fieldName, omit),
				}
				areas = append(areas, area)
			}
		}
