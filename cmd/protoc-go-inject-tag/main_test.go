...
var (
	...
	testInputFileTemp = "./pb/test.pb.temp.go"
)
...
func FuzzParseWriteFile(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, orig []byte) {
		areas, err := parseFile("placeholder.pb.go", orig, nil, false)
		if err == nil {
...
func TestParseWriteFile(t *testing.T) {
	expectedTag := `form:"Id" json:"Id"`

	areas, err := parseFile(testInputFile, nil, nil, false)
	if err != nil {
		t.Fatal(err)
	}
	if len(areas) != 17 {
	...
	newAreas, err := parseFile(testInputFileTemp, nil, nil, false)
...
func TestParseWriteFileClearCommon(t *testing.T) {
	...
	newAreas, err := parseFile(testInputFileTemp, nil, nil, false)
	if len(newAreas) != 16 {
		t.Errorf("parsefile return no equal areas: %d", len(newAreas))
	}
	if err != nil {
		t.Fatal(err)
	}
...
func TestContinueParsingWhenSkippingFields(t *testing.T) {
	...
	areas, err := parseFile(testInputFile, nil, []string{"xml"}, false)
	if err != nil {
		t.Fatal(err)
	}
